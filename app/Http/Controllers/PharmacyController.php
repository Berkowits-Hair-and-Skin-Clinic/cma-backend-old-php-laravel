...
class PharmacyController extends Controller
{
    ...
    public function userpurchaseproducts(Request $request)
    {
        $setting = Setting::find(1);
        //$guest_id="EB4849D4-E4C4-4934-93C4-C70763B2FC70";
        $guest_id=Session::get('zenoti_id');
        $data=Zenoti::purchaseProducts($guest_id);
        $productsData=$data;
        if (!isset($data['page_Info']['total']) || $data['page_Info']['total'] <= 0) {
            // If no record found then redirect to register page
            Session::flash('message',__("No Record Found"));
            Session::flash('alert-class', 'alert-danger');
        ...
        return view("user.patient.purchase")->with("productsData", $productsData)->with("setting", $setting)->with("userdata", $userdata);
