...
public function clintLoginByOTP(Request $request){
 ...
 public function postforgotpassword(Request $request){
    ...
    public function viewspecialist(){
     ...
     public function booking_firsttimeuser($id){
        $setting=Setting::find(1);
        ...
        if(!empty($_REQUEST['finalbookappointment'])){
            ...
            if(isset($_REQUEST['newdate']) && !empty($_REQUEST['newdate']) && isset($_REQUEST['newslot']) && !empty($_REQUEST['newslot']) ){
                $newslotedit=explode("T",$_REQUEST['newslot']);
                $slottime=$_REQUEST['newdate']."T".$newslotedit[1];
        ...
        if(isset($_REQUEST['action'])){
            return view("user.booking_firsttimeuser")->with("therapist_id",$therapist_id)->with("therapist_name",$therapist_id)->with('center_id',$data->center_id)->with("categories",$categories)->with("specilaist",$specilaist)->with("service_name",$service_name)->with("service_id",$service_id)->with('service_date',$service_date)->with("data",$data)->with("setting",$setting)->with("schedule","")->with("token","")->with("getdoctorhoilday","")->with("paymentdetail","")->with("slots",$slots)->with("booking_id",$booking_id);
        }else{
            return view("user.booking_firsttimeuser")->with('center_id',$data->center_id)->with("categories",$categories)->with("specilaist",$specilaist)->with("data",$data)->with("setting",$setting)->with("schedule","")->with("token","")->with("getdoctorhoilday","")->with("booking_id","");
        }
     ...
     public function bookingv2($id){
        ...
        if(!empty($_REQUEST['action'])){
            $therapist_array=explode("|",$_REQUEST['therapist']);
            $therapist_id_1=$therapist_array[0];
            $therapist_name=$therapist_array[1];
            switch($_REQUEST['action']){
                case 'create-booking':
                    if($therapist_id_1=="any"){
                        $therapist_id="any";
                        $service_date=$_REQUEST['date'];
                    }else{
                        $therapist_id=$therapist_id_1;
                        $specilaist=Zenoti::guestDetailsByID($therapist_id);
                    }
                    $service_array=explode("|",$_REQUEST['service']);
                    $service_id=$service_array[0];
                    $service_name=$service_array[1];
     ...
     public function booking($id){
        ...
        // Added by Shak for service and therapist name
        $getdoctorhoilday = Doctor_Hoilday::where("start_date","<=",$date)->where("end_date",">=",$date)->where("doctor_id",$id)->first();
        return view("user.booking")->with("specilaist",$specilaist)->with("services",$services)->with("data",$data)->with("setting",$setting)->with("schedule",$main)->with("token",$token)->with("getdoctorhoilday",$getdoctorhoilday)->with("paymentdetail",$arr)->with("slots",$slots)->with("booking_id",$booking_id);
    }
    ...
    public function searchdoctor_firsttimeuser(Request $request){
        $setting=Setting::find(1);
        $services=Services::all();
        ...
        return view("user.searchcenter_firsttimeuser")->with("cities",$cities)->with("services",$services)->with("setting",$setting)->with("doctorlist",$doctorslist)->with("term",$term)->with("type",$type)->with("doctorslistmap",$doctorslistmap);
    }
    ...
    public function register_firsttimeuser(){
        $setting=Setting::find(1);
        return view("user.patient.register_firsttimeuser")->with("setting",$setting);
    }
