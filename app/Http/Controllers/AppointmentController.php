...
{
    /** For ADMIN DASHBOARD */
    public function video_consultation_list(){
        $center_id=null;
        $recors=Vlc::getAllConsultation($center_id);
        return view("admin.vc")->with('all_consultation',$recors);
    }
    public function ai_analysis_list(){
        $records = AiAnalysis::orderBy('id', 'desc')->get();
        return view("admin.ai-analysis")->with('all_ai_analysis',$records);
    }
    public function payment_by_link(){
        $records = PayByLink::orderBy('id', 'desc')->get();
        return view("admin.payment_by_link")->with('all_payment',$records);
    }
